...
export class StoryController {
  ...
  createStory = async (req, res, next) => {
    try {
      const storyId = await this.stories.createStory(req.body, req.userId);
  ...
  getStory = async (req, res, next) => {
    ...
    try {
      ...
      if (search) {
        const story = await this.stories.getSearchingStory(decodedSearch);
        return res.status(200).json(story);
      }
      const story = await this.stories.getStory(sort);
  ...
  updateStory = async (req, res, next) => {
    ...
    const body = req.body;
    try {
      const story = await this.stories.getStoryUserId(id);
      if (!story) {
      ...
      const storyId = await this.stories.updateStory(id, body);
  ...
  checkAuthor = async (req, res, next) => {
    try {
      const story = await this.stories.getStoryUserId(req.params.id);
